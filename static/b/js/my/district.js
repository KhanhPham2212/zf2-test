                    ...
                    success: function (result) {
                        ...
                        if (result.error == 0) {
                            for (var i = 0; i < result.data.length; i++) {
                                ...
                                var ruralIcon = item.is_rural == 1 ? '<i class="fa fa-check"></i>' : '<i class="fa fa-times"></i>';

                                str += '<tr>';
                                str += '<td style="text-align: center;">' + item.district_id + '</td>';
                                str += '<td class="districtName">' + item.district_name + '</td>';
                                str += '<td style="text-align: center;">' + ruralIcon + '</td>';
                                str += '<td class="ordering">' + item.ordering + '</td>';
                                str += '<td class="isFocus">' + item.is_focus + '</td>';

                                str += '<td style="text-align: center;">';
                                str += '<i class="fa fa-pencil action tooltips edit" style="font-size: 2em;" data-cityid="' + item.city_id + '" data-isrural="' + item.is_rural + '" data-id="' + item.district_id + '" data-original-title="Sửa Quận/ Huyện" data-placement="top"></i>&nbsp;';
                                str += '<i class="fa fa-times-circle tooltips action delete" style="font-size: 2em;" data-id="' + item.district_id + '" data-original-title="Xóa Quận/ Huyện" data-placement="top"></i>';
                                str += '</td>';
                                str += '</tr>';
                            }
...
//                                $("#paginator").html(Pagination(record, totalPage, page));
                            ...
                            str = '<tr><td colspan="5" style="text-align: center;">Chọn Tỉnh / Thành Phố cần xem Quận / Huyện</td></tr>';
                        }
