...
class Ward extends ModelAbstract {
    ...
    public function filter($params) {
        $tmp = array();
        $fields = array('ward_id', 'city_id', 'district_id', 'ward_name', 'ward_slug', 'ordering', 'is_focus', 'is_deleted');
    ...
    public function getListLimit($arrCondition, $intPage, $intLimit, $strOrder) {
        $keyCaching = APPLICATION_ENV . ':' . 'getListLimitWard:' . $intPage . ':' . $intLimit . ':' . str_replace(' ', '', $strOrder) . ':' . $this->cache->read($this->tmpKeyCache);
    ...
    public function getDetail($arrCondition) {
        $keyCaching = APPLICATION_ENV . ':' . 'getDetailWard:';
        if (count($arrCondition) > 0) {
