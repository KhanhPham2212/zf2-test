...
class District extends ModelAbstract {
    ...
    public function filter($params) {
        $tmp = array();
        $fields = array('district_id', 'city_id', 'district_name', 'district_slug', 'is_rural', 'ordering', 'is_focus', 'is_deleted');
    ...
    public function getListLimit($arrCondition = array(), $intPage = 1, $intLimit = 15, $strOrder = 'ordering ASC') {
        $keyCaching = APPLICATION_ENV . ':' . 'getListLimitDistrict:' . $intPage . ':' . $intLimit . ':' . str_replace(' ', '_', $strOrder) . ':' . $this->cache->read($this->tmpKeyCache);
    ...
    public function getDetail($arrCondition = array()) {
        $keyCaching = APPLICATION_ENV . ':' . 'getDetailDistrict:';
        if (count($arrCondition) > 0) {
